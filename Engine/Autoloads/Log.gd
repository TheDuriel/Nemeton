...
var log_to_disk: bool = false # Log to disk?
...
func _ready() -> void:
	...
	if use_builtin_console:
		...
		add_child(layer)
		layer.layer = console_canvas_layer
		var console: LoggerConsole = LoggerConsole.new(self)
		layer.add_child(console)
		console.visible = true
	
	hint(self, "_ready", "logging engaged", "passed")


func hint(emitter: Object, function: String, message: String, result: String = "pass") -> void:
	if enabled and log_level <= LEVELS.HINT:
		_log_message(LEVELS.HINT, emitter, function, message, result)


func warning(emitter: Object, function: String, message: String, result: String = "pass") -> void:
	if enabled and log_level <= LEVELS.WARNING:
		_log_message(LEVELS.WARNING, emitter, function, message, result)


func error(emitter: Object, function: String, message: String, result: String = "pass") -> void:
	if enabled and log_level <= LEVELS.ERROR:
		_log_message(LEVELS.ERROR, emitter, function, message, result)
...
func _log_message(level: int, emitter: Object, function: String, message: String, result: String) -> void:
...
class LoggerConsole:
	...
	func _on_hint_logged(log_text) -> void:
		bbcode_text += "\n" # new_line uses buggy append_bbcode func
		bbcode_text += log_text
	
	
	func _on_warning_logged(log_text) -> void:
		bbcode_text += "\n" # new_line uses buggy append_bbcode func
		bbcode_text += "[color=yellow]" + log_text + "[/color]"
	
	
	func _on_error_logged(log_text) -> void:
		bbcode_text += "\n" # new_line uses buggy append_bbcode func
		bbcode_text += "[color=red]" + log_text + "[/color]"
